...
static unsigned long s(unsigned long s_input)
{
	unsigned long s_output = 0;
	...
	for (j = 0; j < 8; j++) {
		...
		s_output <<= 4;
		s_output |= (unsigned long) (S[j][16*row + column] & 0x0f);
...
static unsigned long s_rocc(unsigned long input_block)
{
...
int main(void)
{
	...
	input = 0x000000000001;

	output1 = s_rocc(input);
	output2 = s(input);
